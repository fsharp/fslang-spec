...
necessary, these sections reference specific inference procedures such as _Name Resolution_ ([§14.1](inference-procedures.md#141-name-resolution))
and _Constraint Solving_ ([§14.5](inference-procedures.md#145-constraint-solving)).
...
- The addition of constraints to the type inference constraint set fails if it causes an inconsistent
    set of constraints ([§14.5](inference-procedures.md#145-constraint-solving)). In this case either an error is reported or, if we are only attempting to
...
Elaborated expressions form the basis for evaluation (see [§6.9](expressions.md#69-evaluation-of-elaborated-forms)) and for the expression trees that
_quoted expressions_ return (see [§6.8](expressions.md#68-quoted-expressions)).
...
- If `field-labeli` is not a single identifier or if the initial type is a variable type, then the field label
    is resolved by performing _Field Label Resolution_ (see [§14.1](inference-procedures.md#141-name-resolution)) on `field-labeli`. This procedure
    ...
    field definition. Each referenced field must be accessible (see [§10.5](namespaces-and-modules.md#105-accessibility-annotations)), as must the type `R`.
...
definitions ([§8.6.3](type-definitions.md#863-additional-object-constructors-in-classes)). For example:
...
2. The type `ty0` must be a class or interface type. The base construction argument `args-expr` must
    ...
    (see [§14.4](inference-procedures.md#144-method-application-resolution)). Except for `ty0`, each `tyi` must be an interface type.
3. The F# compiler attempts to associate each member with a unique _dispatch slot_ by using
    _dispatch slot inference_ ([§14.7](inference-procedures.md#147-dispatch-slot-inference)). If a unique matching dispatch slot is found, then the argument
...
4. The arguments, patterns, and expressions that constitute the bodies of all implementing
    ...
    - If the variable `base-ident` appears, it must be named `base`, and in each member a base
       ...
       values described in [§14.9](inference-procedures.md#149-byref-safety-analysis).

The object must satisfy _dispatch slot checking_ ([§14.8](inference-procedures.md#148-dispatch-slot-checking)) which ensures that a one-to-one mapping
...
result of _dispatch slot checking_ ([§14.8](inference-procedures.md#148-dispatch-slot-checking)).
...
elaborated as a simple for loop-expression ([§6.5.7](expressions.md#657-simple-for-loop-expressions)).
...
([§5.2.2](types-and-type-constraints.md#nullness-constraints), [§5.4.8](types-and-type-constraints.md#548-nullness)) on the initial type of the expression. The constraint ensures that the type directly
...
identifier with respect to the global namespace — that is, ignoring all `open` directives (see [§14.2](inference-procedures.md#142-resolving-application-expressions)). For example:
...
The checking of application expressions is described in detail as an algorithm in [§14.2](inference-procedures.md#142-resolving-application-expressions). To check an
application expression, the expression form is repeatedly decomposed into a _lead_ expression `expr`
and a list of projections `projs` through the use of _Unqualified Lookup_ ([§14.2.1](inference-procedures.md#1421-unqualified-lookup)). This in turn uses
procedures such as _Expression-Qualified Lookup_ and _Method Application Resolution_.

As described in [§14.2](inference-procedures.md#142-resolving-application-expressions), checking an application expression results in an elaborated expression that
...
For uses of active pattern results (see [§10.2.4](namespaces-and-modules.md#1024-active-pattern-definitions-in-modules)), for result `i` in an active pattern that has `N` possible
...
- Otherwise, the type must have one or more accessible constructors. The overloading between
    ...
    (see [§14.4](inference-procedures.md#144-method-application-resolution)).
...
These rules are applied after applying the rules for dynamic operators ([§6.4.4](expressions.md#644-dynamic-operator-expressions)).

The parenthesized operator name is then treated as an identifier and the standard rules for
unqualified name resolution ([§14.1](inference-procedures.md#141-name-resolution)) in expressions are applied. The expression may resolve to a
...
- The `&expr` and `&&expr` address-of operators ([§6.4.5](expressions.md#645-the-addressof-operators))
- The `expr && expr` and `expr || expr` shortcut control flow operators ([§6.5.4](expressions.md#654-shortcut-operator-expressions))
- The `%expr` and `%%expr` expression splice operators in quotations ([§6.8.3](expressions.md#683-expression-splices))
- The library-defined operators, such as `+`, `-`, `*`, `/`, `%`, `**`, `<<<`, `>>>`, `&&&`, `|||`, and `^^^` ([§18.2](the-f-library-fsharpcoredll.md#182-basic-operators-and-functions-fsharpcoreoperators)).

If the operator does not resolve to a user-defined or library-defined operator, the name resolution
rules ([§14.1](inference-procedures.md#141-name-resolution)) ensure that the operator resolves to an expression that implicitly uses a static member
...
written `AddressOf(expr, DefinitelyMutates)`, defined in [§6.9.4](expressions.md#694-taking-the-address-of-an-elaborated-expression).
...
any of the relevant _Name Resolution_ ([§14.1](inference-procedures.md#141-name-resolution)) techniques, including defining the method as a type
...
2. A statically resolved member constraint is applied ([§5.2.3](types-and-type-constraints.md#523-member-constraints)):
...
A modified version of _Unqualified Lookup_ ([§14.2.1](inference-procedures.md#1421-unqualified-lookup)) is applied to the expression `expr1` using a fresh
...
- A reference to a mutable field `expr1a.field` with the actual result type `ty`.
    ...
    Type checking of `expr2` uses the expected result type `ty` and generates an elaborated expression
    `expr2`. The overall elaborated expression is an assignment to a field (see [§6.9.4](expressions.md#694-taking-the-address-of-an-elaborated-expression)):
...
- A array lookup `expr1a.[expr1b]` where `expr1a` has type `ty[]`.
    ...
    expression expr2. The overall elaborated expression is an assignment to a field (see [§6.9.4](expressions.md#694-taking-the-address-of-an-elaborated-expression)):
...
syntax rule for lightweight syntax ([§15.1.1](lexical-filtering.md#1511-basic-lightweight-syntax-rules-by-example)). In practice, this means that code can omit the `;` token
...
syntax rule for lightweight syntax ([§15.1.1](lexical-filtering.md#1511-basic-lightweight-syntax-rules-by-example)).
...
([§6.5.7](expressions.md#657-simple-for-loop-expressions))
...
for lightweight syntax applies ([§15.1.1](lexical-filtering.md#1511-basic-lightweight-syntax-rules-by-example)). For example:
...
syntax rule for lightweight syntax applies ([§15.1.1](lexical-filtering.md#1511-basic-lightweight-syntax-rules-by-example)).
...
lightweight syntax applies ([§15.1.1](lexical-filtering.md#1511-basic-lightweight-syntax-rules-by-example))
...
definitions ([§8.6](type-definitions.md#class-type-definitions)), modules ([§10.2.1](namespaces-and-modules.md#function-and-value-definitions-in-modules)), and computation expressions ([§6.3.10](expressions.md#6310-computation-expressions)), with the following
exceptions:

- Function and value definitions in expressions may not define explicit generic parameters ([§5.3](types-and-type-constraints.md#53-type-parameter-definitions)).
...
- Function and value definitions in expressions are not public and are not subject to arity analysis
    ([§14.10](inference-procedures.md#1410-arity-inference)).
...
1. Check the _value-defn_ ([§14.6](inference-procedures.md#146-checking-and-elaborating-function-value-and-member-definitions)), which defines a group of identifiers `identj` with inferred types `tyj`
...
- If `pat` is a single value pattern `ident`, the resulting elaborated form of the entire expression is
    ...
    where ident1 , typars1 and expr1 are defined in [§14.6](inference-procedures.md#146-checking-and-elaborating-function-value-and-member-definitions).
...
1. Check the `function-defn` ([§14.6](inference-procedures.md#146-checking-and-elaborating-function-value-and-member-definitions)), which defines `ident1`, `ty1`, `typars1` and `expr1`
...
where `ident1` , `typars1` and `expr1` are as defined in [§14.6](inference-procedures.md#146-checking-and-elaborating-function-value-and-member-definitions).
...
are defined, the recursive expressions are analyzed for safety ([§14.6.6](inference-procedures.md#1466-recursive-safety-analysis)). This may result in warnings
...
The expression is first checked as an expression of form `let ident = expr1 in expr2` ([§6.6.1](expressions.md#661-value-definition-expressions)), which results in an elaborated expression of the following form:
...
generalized ([§14.6.7](inference-procedures.md#1467-generalization)). The type `ty1` , is then asserted to be a subtype of `System.IDisposable`. If the
...
[§6.9.24](expressions.md#6924-values-with-underspecified-object-identity-and-type-identity)).
...
- If the method is declared `abstract` — that is, if it is a virtual dispatch slot — then the body of the
    member is chosen according to the dispatch maps of the value of `e0` ([§14.8](inference-procedures.md#148-dispatch-slot-checking)).
...
- The result of evaluation is an object value with union case label `Case` and fields given by the
    values of `e1 , ..., en`.
- If the type `ty` uses null as a representation ([§5.4.8](types-and-type-constraints.md#548-nullness)) and `Case` is the single union case without
...
- The expression evaluates to an object whose runtime type is compatible with all of the `tyi` and
    which has the corresponding dispatch map ([§14.8](inference-procedures.md#148-dispatch-slot-checking)). If present, the base construction expression
...
- The expression is then matched against `pat` to produce a value for each variable pattern ([§7.2](patterns.md#72-named-patterns))
    in `pat`.
...
2. If `v` is `null`, then:
    - If `tye` uses `null` as a representation ([§5.4.8](types-and-type-constraints.md#548-nullness)), the result is `true`.
    - Otherwise the expression evaluates to `false`.
3. If `v` is not `null` and has runtime type `vty` which dynamically converts to `ty` ([§5.4.10](types-and-type-constraints.md#5410-dynamic-conversion-between-types)), the
...
1. Expression `expr` is evaluated to a value `v`.
2. If `v` is `null`:
    - If `tye` uses `null` as a representation ([§5.4.8](types-and-type-constraints.md#548-nullness)), the result is the `null` value.
    - Otherwise a `NullReferenceException` is raised.
3. If `v` is not `null`:
    - If `v` has dynamic type `vty` which _dynamically converts_ to `ty` ([§5.4.10](types-and-type-constraints.md#5410-dynamic-conversion-between-types)), the expression evaluates to the dynamic conversion of `v` to `ty`.
...
> Note: Some F# types — most notably the `option<_>` type — use `null` as a representation
    for efficiency reasons ([§5.4.8](types-and-type-constraints.md#548-nullness)). For these  types, boxing and unboxing can lose type
