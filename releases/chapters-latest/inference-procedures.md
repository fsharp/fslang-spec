...
1. Add each exception label for each exception type definition ([§8.11](type-definitions.md#811-exception-definitions)) in `F` to the _ExprItems_ and
...
Resolution in Expressions_ ([§14.1.4](inference-procedures.md#1414-name-resolution-in-expressions)) except that it consults the _PatItems_ table instead of the _ExprItems_
...
2. If `ident` is not present in the _Types_ table:
    ...
    and Namespace Paths_ ([§14.1.2](inference-procedures.md#1412-name-resolution-in-module-and-namespace-paths)).
...
1. Look up all fields in all available types in the _Types_ table and the _FieldLabels_ table ([§8.4](type-definitions.md#84-record-type-definitions)).
...
2. After decomposition:
    - If `expr` is a long identifier expression `long-ident`, apply _Unqualified Lookup_ ([§14.2.1](inference-procedures.md#1421-unqualified-lookup)) on `long-ident` with projections `projs`.
    - If `expr` is not such an expression, check the expression against an arbitrary initial type `ty`, to
       ...
       _Expression-Qualified Lookup_ ([§14.2.3](inference-procedures.md#1423-expression-qualified-lookup))
...
1. Resolve `long-ident` by using _Name Resolution in Expressions_ ([§14.1](inference-procedures.md#141-name-resolution)). This returns a _name resolution item_ `item` and a _residue long identifier_ `rest`.
...
3. Otherwise, checking proceeds as follows.
    ...
    Instantiate the type scheme of `v`, which results in a type `ty`. Apply these rules:
        ...
        expression.
        - Insert implicit flexibility for the use of the value ([§14.4.3](inference-procedures.md#1443-implicit-insertion-of-flexibility-for-uses-of-functions-and-members)).
    ...
    - If `item` is an implicitly resolved symbolic operator name `op`
        ...
        - The resulting expressions are static member constraint invocation expressions ([§6.4.8](expressions.md#648-member-constraint-invocation-expressions)),
...
1. Inspect `projs` and process according to the following table.
    ...
    | Starts with `(expr2)` | Apply _Function Application Resolution_ ([§14.3](inference-procedures.md#143-function-application-resolution)). | Checking is complete when _Function Application Resolution_ returns. |
...
3. Proceed based on `item` as follows:
    ...
    - If `item` is an instance intermediate language (IL) or F# field `F`
        ...
        - Produce an elaborated form for `expr.F`. If `F` is a field in a value type
          then take the address of `expr` by using the _AddressOf_(`expr, NeverMutates`) operation [§6.9.4](expressions.md#694-taking-the-address-of-an-elaborated-expression).
        - Apply _Expression-Qualified Lookup_ to `fty` and `projs`.
...
7. Choose a unique `M~possible` according to the following rules:
    ...
    ([§14.5](inference-procedures.md#145-constraint-solving)), the method is not applicable. Regardless, the overall constraint set is left unchanged
...
10. Build the resulting elaborated expression by following these steps:
    ...
    _AddressOf_`(obj , PossiblyMutates)` operation ([§6.9.4](expressions.md#694-taking-the-address-of-an-elaborated-expression)).
...
Two additional rules apply when checking arguments (see [§8.13.7](type-definitions.md#8137-type-directed-conversions-at-member-invocations) for examples):
...
- If a formal parameter has delegate type `D`, an actual argument `farg` has known type
  ...
  [§8.13.6](type-definitions.md#8136-optional-arguments-to-method-members).
...
[§5.2](types-and-type-constraints.md#type-constraints) and [§5.4.8](types-and-type-constraints.md#548-nullness).

### 14.5.4 Solving Member Constraints

New constraints in the following form are solved as _member constraints_ ([§5.2.3](types-and-type-constraints.md#523-member-constraints)):
...
As mentioned in [§5.2.3](types-and-type-constraints.md#523-member-constraints), a type variable may not be involved in the support set of more than one
...
3. Each `identi` (of type `tyi`) is then generalized ([§14.6.7](inference-procedures.md#1467-generalization)) and yields generic parameters `<typarsj>`.
...
1. If `ident1` is an active pattern identifier then active pattern result tags are added to the
    environment ([§10.2.4](namespaces-and-modules.md#1024-active-pattern-definitions-in-modules)).
2. The expression `(fun pat1 ... patn : return-type -> rhs-expr)` is checked against a fresh initial
    ...
    does not specify it.
3. The `ident1` (of type `ty1`) is then generalized ([§14.6.7](inference-procedures.md#1467-generalization)) and yields generic parameters `<typars1>`.
4. The following rules are checked:
    ...
    - The patterns of functions may not include optional arguments ([§8.13.6](type-definitions.md#8136-optional-arguments-to-method-members)).
...
([§14.6.6](inference-procedures.md#1466-recursive-safety-analysis)). This analysis may result in warnings—including some reported at compile time—and
runtime checks.

Within recursive groups, each definition in the group is checked ([§14.6.7](inference-procedures.md#1467-generalization)) and then the definitions
...
are permitted ([§14.6.7](inference-procedures.md#1467-generalization)). For example:
...
implicitly reintroduced at each use of the defined function; see [§14.4.3](inference-procedures.md#1443-implicit-insertion-of-flexibility-for-uses-of-functions-and-members)).
...
[§14.9](inference-procedures.md#149-byref-safety-analysis) are treated as implicit declarations of reference cells. For example
