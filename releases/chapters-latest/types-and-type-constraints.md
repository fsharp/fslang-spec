...
with no intervening whitespace, as determined by lexical filtering ([§15.](lexical-filtering.md#15-lexical-filtering)). Specifically:
...
environment_ (see [§14.1](inference-procedures.md#141-name-resolution)), a _floating type variable environment_, which is a mapping from names to type
variables, and a _type inference environment_ (see [§14.5](inference-procedures.md#145-constraint-solving)).

The phrase “fresh type” means a static type that is formed from a _fresh type inference variable_. Type
inference variables are either solved or generalized by _type inference_ ([§14.5](inference-procedures.md#145-constraint-solving)). During conversion and
...
_simplified_ and _eliminated_ based on these equations through _constraint solving_ ([§14.5](inference-procedures.md#145-constraint-solving)).
...
- _Name Resolution for Types_ (see [§14.1](inference-procedures.md#141-name-resolution)) resolves `long-ident` to a type definition with formal generic
...
During checking, _Name Resolution_ (see [§14.1](inference-procedures.md#141-name-resolution)) is applied to the identifier.
...
type inference variable is created (see [§14.5](inference-procedures.md#145-constraint-solving)) and added to both the type inference environment
...
added to the type inference environment (see [§14.5](inference-procedures.md#145-constraint-solving)) for such a type.

A type of the form `^ident` is a _statically resolved type variable_. A fresh type inference variable is
created and added to the type inference environment (see [§14.5](inference-procedures.md#145-constraint-solving)). This type variable is tagged with
an attribute that indicates that it can be generalized only at `inline` definitions (see [§14.6.7](inference-procedures.md#1467-generalization)). The
...
`System.Tuple<_, ..., _>`. (see [§6.3.2](expressions.md#632-tuple-expressions)) for the details of this encoding.
...
in (see [§5.2](types-and-type-constraints.md#52-type-constraints))
...
specified in (see [§5.4.7](types-and-type-constraints.md#547-subtyping-and-coercion))
...
- The implicit use of subsumption when using values and members (see [§14.4.3](inference-procedures.md#1443-implicit-insertion-of-flexibility-for-uses-of-functions-and-members)).
...
specified in (see [§5.4.8](types-and-type-constraints.md#548-nullness))
...
- The `typar` must be a statically resolved type variable of the form `^ident`. This limitation ensures
  ...
  constraint unless that code is marked inline (see [§14.6.7](inference-procedures.md#1467-generalization)).
...
- Each `typar` must be a statically resolved type variable (see [§5.1.2](types-and-type-constraints.md#512-variable-types)) in the form `^ident`. This ensures
  that the constraint is resolved at compile time against a corresponding named type. It also
  means that generic code cannot use this constraint unless that code is marked inline (see [§14.6.7](inference-procedures.md#1467-generalization)).
- The `member-sig` cannot be generic; that is, it cannot include explicit type parameter definitions.
- The conditions that govern when a type satisfies a member constraint are specified in (see [§14.5.4](inference-procedures.md#1454-solving-member-constraints)).
...
the same name, staticness, argument arity, and support set (see [§14.5.4](inference-procedures.md#1454-solving-member-constraints)). If it is, the argument and
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type : (new : unit -> 'T)` is met if `type` has a
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type` : struct is met if `type` is a value type other
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type : not struct` is met if `type` is a reference type.
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type : enum<underlying-type>` is met if `type` is a CLI
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type : unmanaged` is met if `type` is unmanaged as
specified below:
...
During constraint solving (see [§14.5](inference-procedures.md#145-constraint-solving)), the constraint `type : equality` is met if both of the following
conditions are true:
...
(see [§13.1](custom-attributes-and-reflection.md#131-custom-attributes)).
...
- Type definitions may have _custom attributes_ (see [§13.1](custom-attributes-and-reflection.md#131-custom-attributes)), some of which are relevant to checking and
  inference.
- Type definitions may be _type abbreviations_ (see [§8.3](type-definitions.md#83-type-abbreviations)). These are eliminated for the purposes of
  checking and inference (see [§5.4.2](types-and-type-constraints.md#542-expanding-abbreviations-and-inference-equations)).
...
- Type definitions have a `kind` which is one of the following:
  ...
  The kind is determined at the point of declaration by Type Kind Inference (see [§8.2](type-definitions.md#82-type-kind-inference)) if it is not
...
- The inference equations that are inferred from the current inference constraints (see [§14.5](inference-procedures.md#145-constraint-solving)).
- The expansion of type abbreviations (see [§8.3](type-definitions.md#83-type-abbreviations)).
...
and member definitions), the checking process applies _generalization_ (see [§14.6.7](inference-procedures.md#1467-generalization)). Consequently, some
...
generalization to the original source code (see [§14.6.7](inference-procedures.md#1467-generalization)), and the annotation represents the definition site
...
in `Constraint Solving` (see [§14.5](inference-procedures.md#145-constraint-solving)).
...
- **Types with `null` as an abnormal value.** These types do not permit the `null` literal, but do have
  ...
  might include `null` values, see (see [§6.9](expressions.md#69-evaluation-of-elaborated-forms)).
